...
{
    public partial class SuccessPageQP : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string recphtml = "";
            if (txtPGtypeID == 6)
                recphtml = PayuDeails(txtPGtypeID);
            else if (txtPGtypeID == 9 || txtPGtypeID == 4)
                recphtml = CCAvenueDetails(txtPGtypeID);
            HttpContext.Current.Session["RecpHTML"] = recphtml;
        }


        public string PayuDeails(Int64 txtPGtypeID)
        {
            ...
            Paymentchargedt = Paymentchargedt.AsEnumerable().Where(row =>
                row.Field<Int64>("PGID") == txtPGtypeID).CopyToDataTable();
            try
            {
                ...
                {
                    {
                        ...
                        try
                        {
                            ...
                            if (output.ToLower() == "success")
                            {
                                ...
                                string respons = "";
                                respons = objCommon.CallAPI("GetAddMoneyRcptPDFHTML?opID=" + ProductInfo.Tables[0].Rows[0]["OperatorID"].ToString() + "&userID=-1&RcptID=0&RequestType=HTML&PGID=" + RecpPGTransID + "");
        ...
        public string CCAvenueDetails(Int64 txtPGtypeID)
        {
            DataTable Paymentchargedt = (DataTable)HttpContext.Current.Session["PGType"];
            string result = "";
            ...
            try
            {
                ...
                string CancelVar = "true|" + Convert.ToString(ResponseDT.Rows[0]["tracking_id"]) + "|" + Convert.ToString(ResponseDT.Rows[0]["billing_tel"]) + "|" + (ResponseDT.Rows[0]["amount"] == null ? "0" : ResponseDT.Rows[0]["amount"]) + "|" + ResponseDT.Rows[0]["billing_email"] + "|" + ProviderDT.Rows[0]["Operator ID"].ToString() + "|" + txtPGtypeID + "|" + Convert.ToString(ResponseDT.Rows[0]["tracking_id"]) + "|" + Convert.ToString(ResponseDT.Rows[0]["payment_mode"]);
                ...
                if (ResponseDT.Rows[0]["order_status"].ToString() == "Success")
                {
                    ...
                    {
                        clsCommon objCommon = new clsCommon();
                        var response = objCommon.CallAPI("PaymentSave?OPID=" + ProviderDT.Rows[0]["Operator ID"].ToString() +
                            "&CustomerID=" + DsahboardDT.Rows[0]["CustomerID"].ToString() +
                        ...
                        if (output.ToLower() == "success")
                        {
                            ...
                            string respons = "";
                            respons = objCommon.CallAPI("GetAddMoneyRcptPDFHTML?opID=" + ProviderDT.Rows[0]["Operator ID"].ToString().ToString() + "&userID=-1&RcptID=0&RequestType=HTML&PGID=" + RecpPGTransID + "");
                            HttpContext.Current.Session["PGID"] = null;
                            HttpContext.Current.Session["AddMoney"] = null;
                            result = respons;
                        }
            ...
            {
                HttpContext.Current.Session["IsCancel"] = "true|" + Request.Form["tracking_id"] + "|" + Request.Form["billing_tel"].ToString() + "|" + (Request.Form["amount"] == null ? "0" : Request.Form["amount"]) + "|" + Request.Form["billing_email"].ToString() + "|" + ProviderDT.Rows[0]["Operator ID"].ToString() + "|" + txtPGtypeID;
        ...
        public static string GetMyPaymentHistory()//, string Logo
        {
            string result = Convert.ToString(HttpContext.Current.Session["RecpHTML"]);
            return result;
        }
